...
async function isInTransaction(databaseName: string): Promise<boolean> {
  const { sqlite3 } = await maybeInitAsync();
...
async function exec(databaseName: string, source: string) {
  const { sqlite3 } = await maybeInitAsync();
  const db = databases.get(databaseName);
  if (!db) throw new Error(`Database ${databaseName} not found`);
...
async function prepare(databaseName: string, source: string): Promise<ResultTypeMap['prepare']> {
  const { sqlite3 } = await maybeInitAsync();
  const db = databases.get(databaseName);
  if (!db) throw new Error(`Database ${databaseName} not found`);
  ...
  try {
    ...
    if (!statement) throw new Error('Failed to prepare statement');
...
function bindStatementParam(sqlite3: SQLiteAPI, stmt: Statement, param: any, index: number) {
  if (param == null) {
...
function getColumnValues(sqlite3: SQLiteAPI, stmt: Statement): SQLiteColumnValues {
...
function getColumnValue(sqlite3: SQLiteAPI, stmt: Statement, index: number): any {
...
async function step(databaseName: string, statementId: number): Promise<SQLiteColumnValues | null> {
  const { sqlite3 } = await maybeInitAsync();
  const db = databases.get(databaseName);
  if (!db) throw new Error(`Database ${databaseName} not found`);
  const stmt = statements.get(statementId);
...
async function reset(databaseName: string, statementId: number): Promise<void> {
  const { sqlite3 } = await maybeInitAsync();
  const db = databases.get(databaseName);
  if (!db) throw new Error(`Database ${databaseName} not found`);
  const stmt = statements.get(statementId);
...
async function getColumnNames(statementId: number): Promise<SQLiteColumnNames> {
  const { sqlite3 } = await maybeInitAsync();
  const stmt = statements.get(statementId);
  if (!stmt) throw new Error(`Statement ${statementId} not found`);
  const columnCount = sqlite3.column_count(stmt);
...
async function finalize(databaseName: string, statementId: number): Promise<void> {
  const { sqlite3 } = await maybeInitAsync();
  const db = databases.get(databaseName);
  if (!db) throw new Error(`Database ${databaseName} not found`);
  const stmt = statements.get(statementId);
